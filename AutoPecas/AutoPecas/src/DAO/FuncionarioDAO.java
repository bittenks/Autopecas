...
public class FuncionarioDAO {
    ...
    public List<Funcionario> ListaFuncionarioNome(String nomeUsuario) throws SQLException{
        List <Funcionario> listaFuncionarios;
        listaFuncionarios = new ArrayList<>();
        sql = "select * from usuario where nomeUsuario like '%" + nomeUsuario + "%'";
        pst = Conexao.getInstance().prepareStatement(sql);
        ResultSet rs = pst.executeQuery();
        while (rs.next()){
            listaFuncionarios.add(new Funcionario (rs.getInt("idUsuario"), rs.getString("nomeUsuario"), rs.getString("cpfUsuario"),
            rs.getString("telUsuario"), rs.getString("funcaoUsuario")));
        }
        pst.close();
        return listaFuncionarios;
    }
